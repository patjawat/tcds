...
class PatientHelper extends Component {
    ...
    /**
     * ข้อมูลตามการรับผู้ป่วยได้แก่ HN คำนำหน้า ชื่อ นามสกุล เพศ อายุ แพทย์ผู้รักษา
     * @param type $hn
     * @return string
     */
    public static function getPatientTitleByHn($hn) {
        if (empty($hn)) {
            $title = '<i class="fa fa-wheelchair" aria-hidden="true"></i> กรุณาเลือกผู้เข้ารับบริการ XXX';
        } else {
    ...
    /**
     * ข้อมูลเพศ M = ชาย , F = หญิง
     * @return string
     */
    public static function getCurrentSex() {
        return (\Yii::$app->session->get('sex') === "M" ? "ชาย" : (\Yii::$app->session->get('sex') === "F" ? "หญิง" : "---"));
    }
